...
LineCountWebParticipant = 8;
// Количество строк официальных представителей
LineCountWebOfficial = 8;
// Показать время 
...
const FS_NameLineParticipant = (data) => `<div class='row LineRow'><div class="col-1 LineSort">${data["Sort"]}</div><div class="col LineText">${data["FullName"]} <small>  ${data['Status']}</small><span class="Nation">${data["City"]}</span></div></div>`;
/* Шаблон: Списка участников с оценками */
const FS_3SCLineParticipant = (data) => `<div class='row LineRow ${data["CurrentClass"]}'><div class="col-1 LineSort">${data["Sort"]}</div><div class="col LineText">${data["FullName"]}<span class="Nation">${data["City"]}</span></div><div class='col-2 LineLast participantPoint'>${data["Point"]}</div></div>`;
/* Шаблон: Пустая строка между участниками */
const FS_LineTextEmpty = `<div class='row LineRow'><div class="col LineTextEmpty">.....</div></div>`;
/* Шаблон: Списка участников с оценками */
const FS_IRSLineParticipant = (data) => `<div class='row LineRow ${data["CurrentClass"]}'><div class="col-1 LineSort">${data["Sort"]}</div><div class="col LineText">${data["FullName"]}<span class="Nation">${data["City"]}</span></div><div class='col-2 LineLast participantPoint'>${data["Point"]}</div></div>`;
/* Шаблон: Списка участников с оценками */
const FS_RESLineParticipant = (data) => `<div class='row LineRow ${data["CurrentClass"]}'><div class="col-1 LineSort">${data["Sort"]}</div><div class="col LineText">${data["FullName"]}<span class="Nation">${data["City"]}</span></div><div class='col-2 LineLast participantPoint'>${data["Point"]}</div></div>`;
