 ...
 */
 const AdminApp = {
    ...
    methods: {
		saveTeamList() {
			this.SendOrGetData('SaveTeamList', {'Value': {
				'Key': this.TeamSelected,
				'Team': this.Team
			}});
		},
		deleteTeamList() {
			this.SendOrGetData('DeleteTeamList', {'Value': this.TeamSelected});
			this.TeamSelected = 0;
		},
		createTeamList() {
			this.SendOrGetData('CreateTeamList', {'Value': false});
		},
		getTeam(TeamUID) {
			if (this.TeamSelected != 0) {
				this.SendOrGetData('GetTeam', {'Value': TeamUID});
			}
		},
		removeRow (index) {
	...
	mounted() {
		this.SendOrGetData("GetTeamsList");
	}
};
